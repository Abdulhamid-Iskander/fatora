...
import '../models/product_installment.dart';
import '../database_helper.dart';

class ProductInstallmentsPage extends StatefulWidget {
  const ProductInstallmentsPage({Key? key}) : super(key: key);

  @override
  State<ProductInstallmentsPage> createState() =>
      _ProductInstallmentsPageState();
}

class _ProductInstallmentsPageState extends State<ProductInstallmentsPage> {
  List<ProductInstallment> _installments = [];
  ...
  Future<void> _loadInstallments() async {
    ...
    try {
      final installmentsData = await _databaseHelper.getProductInstallments();
      final loadedInstallments = installmentsData.map((installmentMap) {
        return ProductInstallment.fromMap(installmentMap);
      }).toList();
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error loading product installments: $e'),
  ...
  List<ProductInstallment> get _filteredInstallments {
  ...
  Future<void> _markAsPaid(ProductInstallment installment) async {
    try {
      await _databaseHelper.markProductInstallmentAsPaid(
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Product installment marked as paid!'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error updating product installment: $e'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  void _showInstallmentDetails(ProductInstallment installment) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text('Product Installment #${installment.installmentNumber}'),
          content: SingleChildScrollView(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              mainAxisSize: MainAxisSize.min,
              children: [
                _buildDetailRow('Product', installment.productName),
                _buildDetailRow('Product ID', installment.productId),
                _buildDetailRow(
  ...
  Color _getInstallmentColor(ProductInstallment installment) {
  ...
  IconData _getInstallmentIcon(ProductInstallment installment) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Product Purchase Installments',
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return Card(
                              ...
                              child: ListTile(
                                ...
                                title: Text(
                                  '${installment.productName} - Installment #${installment.installmentNumber}',
                                  style: const TextStyle(
                                    fontWeight: FontWeight.bold,
                                  ),
                                ),
  ...
  void _editInstallment(ProductInstallment installment) {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text(
              'Edit Product Installment #${installment.installmentNumber}'),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              TextFormField(
              ...
              onPressed: () async {
                ...
                try {
                  await _databaseHelper
                      .updateProductInstallment(updatedInstallment.toMap());
                  await _loadInstallments();
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content:
                            Text('Product Installment updated successfully!'),
                        backgroundColor: Colors.green,
                      ),
                    );
                  }
  ...
  void _deleteInstallment(String id) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('Delete Product Installment'),
          content: const Text(
              'Are you sure you want to delete this product installment?'),
          actions: [
            TextButton(
              onPressed: () {
                Navigator.of(context).pop();
            ...
            ElevatedButton(
              onPressed: () async {
                try {
                  await _databaseHelper.deleteProductInstallment(id);
                  await _loadInstallments();
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content:
                            Text('Product Installment deleted successfully!'),
                        backgroundColor: Colors.green,
                      ),
                    );
                  }
                  ...
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('Error deleting product installment: $e'),
                        backgroundColor: Colors.red,
                      ),
                    );
                  }
